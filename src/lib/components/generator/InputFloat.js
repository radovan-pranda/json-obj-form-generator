...
export class InputFloat extends Component {
    onChange = function (e) {
        ...
        var value = {
            value: (err.err_type && val !== null && val.length > 0)?filterFloat(String(val)):val,
            errors: err
        };
    ...
    render() {
        return (
            <Fragment>
                ...
                <FormText hidden={this.props.errors.warn_def} color="info" className="jofgen-warning jofgen-warning-float">{this.props.warn_def + ' ' + this.props.default}</FormText>
                <FormText hidden={this.props.errors.err_type} color="danger" className="jofgen-warning jofgen-warning-float">{this.props.err_type}</FormText>
                <FormText hidden={this.props.errors.err_req} color="danger" className="jofgen-warning jofgen-warning-float">{this.props.err_req}</FormText>
                <FormText hidden={this.props.errors.err_min} color="danger" className="jofgen-warning jofgen-warning-float">{this.props.err_min}</FormText>
                <FormText hidden={this.props.errors.err_max} color="danger" className="jofgen-warning jofgen-warning-float">{this.props.err_max}</FormText>
                <FormText hidden={this.props.errors.err_inf} color="danger" className="jofgen-warning jofgen-warning-float">{this.props.err_inf}</FormText>
            </Fragment>
        );
    }
}
...
InputFloat.defaultProps = {
    ...
    err_type: "Invalid type of value. Value is not float number.",
...
export const getErrors = function (e, props) {
    ...
    if (new_value !== undefined && new_value !== null) {
        var filter_new_value = filterFloat(new_value);
        ...
        var e_mxl = !floatValid(max) || !new_value_valid || filter_new_value <= filterFloat(max);
...
export const getValue = function (e, props) {
    if (props.required) {
        ...
        {
            return (floatValid(e))?filterFloat(e):e;
        }
    }
    else {
