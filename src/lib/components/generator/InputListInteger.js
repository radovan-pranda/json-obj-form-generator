  ...
  onChange = function (e) {
    ...
    switch (e.target.name) {
      case "value-new":
        ...
        err.new_value = (err.new_type && e.target.value.length > 0) ? filterInt(String(e.target.value)) : e.target.value;
        break;
      ...
      default:
        ...
        val[idx] = (err.etype[idx] && e.target.value.length > 0) ? filterInt(String(e.target.value)) : e.target.value;
    }
...
const valid = function (value, props) {
  var valid = [];
  var iValue = filterInt(String(value));
  var validValue = intValid(String(value));
