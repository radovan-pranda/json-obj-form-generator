...
import { ListIntegerTranslationPropType as translationProp, Default_ListIntegerTranslation as default_translationProp, Default_keyPropType, keyPropType } from './propTypes';
...
export class InputListInteger extends Component {
  state = {
    isOpenMain: true,
    invalid: true,
    errPopUp: false,
    uidErrorShow: false,
    gId: idGenerator(this.props.jkey.prefix + "il-int" + this.props.jkey.sufix),
  }
  ...
  onChange = function (e) {
    ...
    if (["min", "max", "minNo", "maxNo"].includes(e.target.name)) {
      props.value = validGetArrays(props.value, props, intValid, filterInt);
      props.default = validGetArrays(props.default, props, intValid, filterInt);
    }
  ...
  render() {
    ...
    /* ------ validation ------ */
    var fmax = filterInt(this.props.value.max);
    var fmin = filterInt(this.props.value.min);
    var mnL = intValid(this.props.value.min);
    var mxL = intValid(this.props.value.max);
          ...
          <FormGroup row>
            ...
            <Col className="jofgen-D-input-col">
              <ListNumeric
                ...
                min={(intValid(this.props.value.min) && rangeValid && fmin >= 0) ? filterInt(this.props.value.min) : undefined} max={(intValid(this.props.value.max) && rangeValid && fmax >= 0) ? filterInt(this.props.value.max) : undefined}
    ...
    return (<div className={this.props.className + " jofgen-D-card jofgen-D-ilInt" + ((this.props.value.required) ? " required" : "") + ((invalid || this.props.invalid) ? " invalid" : "")} style={this.props.style} >
...
export const valid = function (e) {
  ...
  var fminL = filterInt(e.min);
  var fmaxL = filterInt(e.max);
  var vdl = e.default.values.length;
  var vvl = e.value.values.length;

  var vml = intValid(e.min);
  var vmxl = intValid(e.max);
...
export const clean = function (e) {
  ...
  var minL = filterInt(e.min);
  var maxL = filterInt(e.max);
  ...
  var minLValid = intValid(e.min) && (maxL >= minL || isNaN(maxL));
  var maxLValid = intValid(e.max) && (maxL >= minL || isNaN(minL));
...
export const rebuild = function (e) {
  return {
    ...
    value: (e.value !== undefined && e.value !== null) ? list_rebuild(e.value, e, intValid, filterInt) : ListNumeric_prototype(),
    default: (e.default !== undefined && e.default !== null) ? list_rebuild(e.default, e, intValid, filterInt) : ListNumeric_prototype(),
